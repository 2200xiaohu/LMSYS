...
def train(args):
    ...
    if args.use_cache and os.path.exists(train_cache_path):
        ...
        tokenized_dataset = MultiTurnDataSet(df_train, tokenizer, args.MAX_INPUT, 1)
    ...
    if args.use_cache and os.path.exists(valid_cache_path):
        ...
        tokenized_dataset_valid = MultiTurnDataSet(df_valid, tokenizer, args.MAX_INPUT, 1)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Demo of argparse')
    parser.add_argument('--config', default='train_llama_multi_turn.yaml', type=str, help='Path to the config file', required=False)
    args = parser.parse_args()
