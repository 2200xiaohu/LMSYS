...
class CustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        label_loss_fct = nn.CrossEntropyLoss(weight=weights)
        eos_loss_fct = nn.CrossEntropyLoss()
        #print(f"Logits shape: {logits.shape}, Labels shape: {labels.shape}")
        label_loss = label_loss_fct(label_logits.view(-1, 3), label_label.view(-1))
        eos_loss = eos_loss_fct(eos_logits.view(-1, vocab_size), eos_label.view(-1))
        loss = label_loss + eos_loss
...
def train(args):
    ...
    wandb.save("train_gemma2_instruction_cls_loss.py")
    wandb.save("train_gemma2_instruction_cls_loss.yaml")
    # HUGGING FACE MODEL
    MODEL = args.MODEL
    ...
    global weights
    weights = torch.tensor([1/i for i in df_train.label.value_counts().tolist()]).cuda()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Demo of argparse')
    parser.add_argument('--config', default='train_gemma2_instruction_cls_loss.yaml', type=str, help='Path to the config file', required=False)
    args = parser.parse_args()
